...
//    public List<TreeNode> test(List<TreeNode> nodes, double n) {
//        return nodes.stream().map(node -> {
//            if (!CollectionUtils.isNotEmpty(node.getChildren())) {
//                node.setValue(test(node.getChildren(), node.getValue()));
//                long count = vehicleTypeNode.getNodes().stream()
//                        .mapToLong(a -> a.getText().getAllCount()).sum();
//            }
